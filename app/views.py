    ...
    if not ko:
        ...
        if sl == 'ur1':
            ...
            if tem == 'slozh-umn-ver':
                run = random.choice(['z1','z2','z3'])
                if run=='z1':
                    m = ['Вероятность хотя бы одного попадания в цель при ', ' выстрелах равна ',' Найти вероятность попадания в цель при одном выстреле.']
                ...
                if run=='z3':
                    ...
                    for i in range(n):
                        f *= 1 - a[i]
                    f = f / (1 - a[0])
                    s1 = str(n) + ' учащихся на экзамене независимо друг от друга решают одну и ту же задачу. Вероятности ее решения этими учащимися равны:\n'
                    for i in range(n):
                        s1 += str(a[i]) + '\n'
            ...
            if tem == 'bayes':
                ...
                if run == 'z1':
                    ...
                    problemCondition = f'Вы разрабатываете систему фильтрации спама для электронной почты. Известно, что {p_a * 100}% всех отправленных писем являются спамом. Ваша система имеет точность {p_b_given_a * 100}%, то есть она правильно помечает {p_b_given_not_a}% спам-писем как спам, и 5% легитимных писем помечает как спам. Определенное письмо было помечено как спам вашей системой. Какова вероятность того, что оно действительно является спамом?'
                ...
                if run == 'z2':
                    ...
                    problemCondition = f'Есть определенная болезнь, которая поражает {int(diseaseProb * 100)}% населения. Существует тест, который правильно распознает болезнь в {int(diseasePositiveProb * 100)}% случаев, но также дает ложноположительные результаты в {int(diseasePositiveNotProb * 100)}% случаев на здоровых людях. Если человек получил положительный результат теста, какова вероятность того, что он действительно болен?'
                ...
                if run == 'z3':
                    ...
                    # ----------------------------
                    # Найдем вероятность принадлежности стрелка к той или иной группе
                    ...
                    # ----------------------------
                    # введем событие A = Стрелок попал в мишень.
                    # Выпишем условные вероятности P(A | H_i):
                    ...
                    # ----------------------------
                    # Теперь найдем апостериорные вероятности того, что
                    # стрелок принадлежал к i-ой группе, если он попал в цель
                    # по формуле Байеса
                    ...
                    problemCondition = f'Из {total} стрелков {groupOne} попадает в цель с вероятностью {shooterInGroupOne}, {groupTwo} - с вероятностью {shooterInGroupTwo} и {groupThree} – с вероятностью {shooterInGroupThree}. Наудачу выбранный стрелок произвел выстрел, поразив цель. К какой из групп вероятнее всего принадлежал этот стрелок?'

                    s1 = f'\n{problemCondition} Ответ: {maxNum}'
            if tem == 'bern':
                run = random.choice(['z1', 'z2', 'z3'])
                if run == 'z1':
                    z = ["Пусть проводится n = "," независимых испытаний, в каждом из которых вероятность появления события A постоянна и равна p = ",". Найти вероятность того, что в данной серии испытаний событие A появится m = ", " раза."]
                ...
                if run == 'z2':
                    z = ["Пусть вероятность того, что телевизор потребует ремонта в течение гарантийного срока, равна ",". Найти вероятность того, что в течение гарантийного срока из "," телевизоров: а) не более одного потребует ремонта; б) хотя бы один не потребует ремонта."]
                    n = random.randint(2, 10)
                ...
                if run == 'z3':
                    z = ["Семья собирается купить "," kinder сюрприза. Вероятность того, что сюрпризом будет машинка, равна ",". Вероятность того, что сюрпризом будет кукла, равна ", ". Какова вероятность того, что из "," Kinder Сюрпризов сын может достать ровно ", " машинок?"]
                    n = random.randint(2, 10)
                    p = round(random.uniform(0.1, 1), 2)
                    q = round(1 - p, 2)
            ...
            if tem == 'naiv':
                ...
                if run == 'z2':
                    z = ['При автоматической наводке орудия вероятность попадания по быстро движущейся цели равна ','. Найти наивероятнейшее число попаданий при ', ' выстрелах. ']
                    p = (random.randint(1, 99)) / 100
                ...
                if run == 'z3':
                    z = ['Данные длительной проверки качества выпускаемых стандартных деталей показали, что в среднем брак составляет ','%. Определить наиболее вероятное число вполне исправных деталей в партии из ', ' штук. ']
            ...
            if tem == 'muavr-laplas':
                ...
                if run == 'z2':
                    m = ['На конвейер за смену поступает  ',' изделий. Вероятность того, что поступившая на конвейер деталь стандартна, равна  ','. Найти вероятность того, что стандартных деталей на конвейер за смену поступило ровно ']
            ...
            if tem == 'int-muavr-laplas':
                ...
                if run == 'z2':
                    z = ['Страховая компания заключила ',' договоров. Вероятность страхового случая по каждому из них в течение года составляет ','%. Найти вероятность, что таких случаев будет не более ', '. ']
                ...
                if run == 'z3':
                    z = ['Вероятность выхода из строя за смену одного станка равна ','. Определить вероятность выхода из строя от ', ' до ', ' станков при наличии ', ' станков. ']
            ...
            if tem == 'puass':
                run = random.choice(['z1', 'z2','z3'])
                if run == 'z1':
                    z = ['Станок штампует детали. Вероятность того, что изготовленная деталь бракованная, равна ','. Какова вероятность того, что среди ', ' деталей окажется ', ' бракованных']
                ...
                if run == 'z2':
                    z = ['Автобиография писателя издается тиражом в ',' экземпляров. Для каждой книги вероятность быть неправильно сброшюрованной равна ','. Найти вероятность того, что тираж содержать ровно ', ' бракованных книг.']
            ...
            if tem == 'diskr':
                ...
                if run == 'z2':
                    z = ['Охотник стреляет по дичи до первого попадания, но успевает сделать не более четырех выстрелов. Вероятность попадания в цель при одном выстреле равна ',' . Найти дисперсию этой случайной величины.']
                ...
                if run == 'z3':
                    z = ['Стрелок, имея 3 патрона, стреляет в цель до первого попадания. Вероятности попадения при первом, втором и третьем выстрелах соответственно ',', ', ' и ', '. Чему равно математическое ожидание числа оставшихся патронов?']
            ...
            if tem == 'nepr':
                ...
                if run =='z1':
                    ...
                    problemCondition = f'Пусть X - случайная величина с экспоненциальным распределением с параметром lambda = {lambdaValue}. Найдите вероятность того, что X примет значение меньше или равное {x[1]}.'
                ...
                if run == 'z2':
                    ...
                    problemCondition = f'Пусть X - случайная величина с треугольным распределением на интервале [{a}, {b}] с максимальным значением в точке {c}. Найти математическое ожидание и дисперсию этой случайной величины.'
                ...
                if run == 'z3':
                    ...
                    problemCondition = f'Пусть X - случайная величина с равномерным распределением на интервале [{a}, {b}]. Найти среднее значение (математическое ожидание) и дисперсию этой случайной величины.'
                    s1 = f'\n{problemCondition} Ответ: {mean} {variance}'
            if tem == 'norm':
                run = random.choice(['z1', 'z2', 'z3'])
                if run == 'z1':
                    z = ['Размер детали задан полем допуска ', '-', ' мм. Оказалось, что средний размер деталей равен ',' мм, а квадратичное отклонение ',' мм. Считая, что размер детали подчиняется закону нормального распределения, определить вероятность появления брака.']
                ...
                if run == 'z2':
                    z = ['Требуется найти вероятность того, что нормально распределённая случайная величина, где a = ',' - математическое ожидание, σ = ',' - среднее квадратичное отклонение случайной величины X, принимает значения: меньше ']
                ...
                if run == 'z3':
                    z = ['Случайная величина X имеет нормальное распределение с математическим ожиданием a = ',' и дисперсией D(x) = ', '. Найти вероятность попадания этой случайной величины на интервал (',',', '). ']
                    a = random.randint(1, 10)
        ...
        if sl == 'ur2':
            if tem == 'klasopr':
                run = random.choice(['z1', 'z2', 'z3'])
                if run == 'z1':
                    z = ['В лифт ', '-этажного дома на первом этаже зашли ',
                         ' человека. И поехали. Найти вероятность того, что они выйдут на разных этажах.']
                ...
                if run == 'z2':
                    z = ['В лифт ', '-этажного дома на первом этаже зашли ',
                         ' человека. И поехали. Найти вероятность того, что они выйдут на одном этаже.']
                    n = 100000
                    while (n > 9000):
                        p = random.randint(6, 20)
                        h = random.randint(2, 5)
                    ...
                    s1 = z[0] + str(p) + z[1] + str(h) + z[2] + ' Ответ: ' + str(f)
                if run == 'z3':
                    z = ['Подбрасывается k = ', ' монет. Найти вероятность того, что на ',
                         ' монетах выпадет орёл, а на остальных – решка.']
            ...
            if tem == 'slozh-umn-ver':
                run = random.choice(['z1', 'z2', 'z3'])
                if run == 'z1':
                    # Два игрока A и B поочередно бросают монету. Выигравшим считается тот, у
                    # кого раньше выпадет герб. Первый бросок делает игрок A, второй – B, третий – A и т.д.
                    # Найти вероятность того, что A выиграл до k броска. Ответ округлить до 4 знака после запятой.
                    k = np.random.randint(3, 8)  # количество бросков при короткой игре

                    # создадим функцию probOfWinning, которая будет возвращать вероятность выигрыша
                    ...
                    # Вероятность того, что А выиграет на втором броске равна 0.5**3 (при первом броске у А
                    # выпадет решка – 0.5, при первом броске у В выпадет решка – 0.5, при втором броске у А
                    # выпадет герб – 0.5).
                    # Аналогично, вероятность того, что А выиграет на k броске равна = 0.5**(2*n-1), где n - номер попытки.
                    def probOfWinning(attempt):
                        return 0.5 ** (2 * attempt - 1)

                    # Тогда вероятность того, что А выиграет до k броска равна сумме вероятностей того, что А
                    # выиграет на 1 броске, на 2 броске, …, на k-1 броске.
                    P, n = 0, 1
                    while (n < k):
                        P += probOfWinning(n)
                        n += 1
                    problemCondition = f"""Два игрока A и B поочередно бросают монету. Выигравшим считается тот, у кого раньше выпадет герб. Первый бросок делает игрок A, второй – B, третий – A и т.д. Найти вероятность того, что A выиграл до {k} броска. Ответ округлить до 3 знака после запятой."""
                    ans = f" Ответ: {np.round(P, decimals=3)}"
                    s1 = problemCondition + ans
                if run == 'z2':
                    # Генерация случайных значений переменных
                    ...
                    qualifiedProgrammers = np.random.randint(1,
                                                             numOfProgrammers + 1)  # число квалифицированных программистов
                    ...
                    minimumProgrammersQualified = np.random.randint(1,
                                                                    3)  # минимальное число квалифицированных программистов
                    # Проверка, чтобы minimumAuditorsQualified не было больше qualifiedAuditors
                    if minimumAuditorsQualified > qualifiedAuditors:
                        minimumAuditorsQualified = qualifiedAuditors
                    # Проверка, чтобы minimumProgrammersQualified не было больше qualifiedProgrammers
                    if minimumProgrammersQualified > qualifiedProgrammers:
                        minimumProgrammersQualified = qualifiedProgrammers

                    # Введем события:
                    # X = (В группе окажется, по крайней мере, qualifiedAuditors аудиторов высокой квалификации и хотя бы
                    # qualifiedProgrammers программистов высокой квалификации).
                    # Введем вспомогательные независимые события:
                    # A = (В группе окажется, по крайней мере, qualifiedAuditors аудитор высокой квалификации),
                    # B = (В группе окажется хотя бы qualifiedProgrammers программист высокой квалификации),
                    # Так что искомое событие можно представить как X = A * B, P(X) = P(A)*P(B)
                    def probabilityCalc():  # Вычисление вероятности
                        auditorsNeeded, auditorsNotHighQualified = [
                            comb(numOfAuditors, auditorsGroup),
                            # число способов выбрать число auditorsGroup из общего числа аудиторов
                            comb(numOfAuditors - qualifiedAuditors, auditorsGroup)
                            # число способов выбрать аудиторов не высшей квалификации
                        ]

                        programmersNeeded, programmersNotHighQualified = [
                            comb(numOfProgrammers, programmersGroup),
                            # число способов выбрать число programmersGroup из общего числа программистов
                            comb(numOfAuditors - qualifiedProgrammers, programmersGroup)
                            # число способов выбрать программистов не высшей квалификации
                        ]
                    ...
                    problemCondition = f"""В фирме работают {numOfAuditors} аудиторов, из которых {qualifiedAuditors} – высокой квалификации, и {numOfProgrammers} программистов, из которых {qualifiedProgrammers} высокой квалификации. В командировку надо отправить группу из {auditorsGroup} аудитор(-ов) и {programmersGroup} программист(-ов). Какова вероятность того, что в этой группе окажется по крайней мере {minimumAuditorsQualified} аудитор(-ов) высокой квалификации и хотя бы {minimumProgrammersQualified} программист(-ов) высокой квалификации, если набор группы проводился анонимным анкетированием и каждый специалист имел равные возможности поехать в командировку?"""
                ...
                if run == 'z3':
                    ...
                    problemCondition = f"""Экспедиция издательства отправила газеты в три почтовых отделения. Вероятность своевременной доставки газет в первое отделение равна {PA1}, во второе - {PA2}, в третье - {PA3}. Найти вероятность следующих событий: а) только одно отделение получит газеты вовремя; б) хотя бы одно отделение получит газеты с опозданием."""
            ...
            if tem == 'fpv':
                ...
                if run == 'z1':
                    z = [
                        ...
                        '%. Консультационная фирма, занимающаяся прогнозом рыночной ситуации, подтвердила предположение о росте спроса. Положительные прогнозы консультационной фирмы сбываются с вероятностью ',
                ...
                if run == 'z3':
                    ...
                    g = b / 100
                    h = c / 100
                    s1 = z[0] + str(a) + z[1] + str(b) + z[2] + str(c) + z[3] + str(n) + z[4] + str(m) + z[5] + str(k) + z[6] + ' Ответ: ' + str(round((n / (n + m + k)) * f + (m / (n + m + k)) * g + (k / (n + m + k)) * h, 3))
            if tem == 'bayes':
                run = random.choice(['z1', 'z2', 'z3'])
                if run == 'z1':
                    z = [
                    ...
                    s1 = z[0] + str(n1) + z[1] + str(n2) + z[2] + str(int(b1 * 100)) + z[3] + str(int(b2 * 100)) + z[4] + str(int(b3 * 100)) + z[5] + ' Ответ: ' + str(p)
                if run == 'z2':
                ...
                if run == 'z3':
                    z = [
                        'Компания по страхованию автомобилей разделяет водителей по трѐм классам: класс А (мало рискует), класс В (рискует средне), класс С (рискует сильно). Компания предполагает, что из всех водителей, застрахованных у неѐ, ',
                        ...
                        '. Мистер Джонс страхует свою машину у этой компании и в течение года попадает в автокатастрофу. Какова вероятность того, что он относится к классу А?']
            ...
            if tem == 'bern':
                ...
                if run == 'z3':
                    z = ['В результате обследования были выделены семьи, имеющие по ',
                         ' ребенка. Считая вероятности появления мальчика и девочки в семье равными, определить вероятности появления в ней: а) ',
                         ' мальчика, б) ', ' девочек.']
            ...
            if tem == 'naiv':
                ...
                if run=='z1':
                    ...
                    s1 = 'Испытывается каждый из ' + str(
                        n) + ' элементов некоторого устройства. Вероятность того, что элемент выдержит испытание, равна ' + str(
            ...
            if tem == 'diskr':
                ...
                if run == 'z1':
                    ...
                    d = 1 - h
                    e = 1 - j
                    s1 = z[0] + str(f) + z[1] + str(g) + z[2] + ' Ответ: ' + str(round((0 * c * d * e) + (1 * (f * d * e + c * h * e + c * d * j)) + (2 * (c * h * j + f * d * j + f * h * e)) + (3 * f * h * j), 3)) + ' ' + str(round((0 * c * d * e) + (1 * (f * d * e + c * h * e + c * d * j)) + (4 * (c * h * j + f * d * j + f * h * e)) + (9 * f * h * j) - ((0 * c * d * e) + (1 * (f * d * e + c * h * e + c * d * j)) + (2 * (c * h * j + f * d * j + f * h * e)) + (3 * f * h * j)) ** 2,3)) + ' ' + str(round(((0 * c * d * e) + (1 * (f * d * e + c * h * e + c * d * j)) + (2 * (c * h * j + f * d * j + f * h * e)) + (3 * f * h * j)) ** (1 / 2), 3))
                if run == 'z2':
                    z = [
                    ...
                    e = 1 - b
                    f = 1 - c
                    s1 = z[0] + str(a) + z[1] + str(b) + z[2] + str(c) + z[3] + ' Ответ: ' + str(round(0 * (d * e * f) + 1 * (a * e * f + d * b * f + d * e * c) + 2 * (a * b * f + a * e * c + d * b * c) + 3 * (a * b * c), 3)) + ' ' + str(round((0 * (d * e * f) + 1 * (a * e * f + d * b * f + d * e * c) + 4 * (a * b * f + a * e * c + d * b * c) + 9 * (a * b * c)) - (0 * (d * e * f) + 1 * (a * e * f + d * b * f + d * e * c) + 2 * (a * b * f + a * e * c + d * b * c) + 3 * (a * b * c)) ** 2,3))
                if run == 'z3':
                    z = [
                        'В ящике содержится m стандартных и n бракованных детали. Вынимают детали последовательно до появления стандартной, не возвращая их обратно. Случайной дискретной величиной является число извлеченных бракованных детелей. Вычислить математическое ожидание, дисперсию, среднее квадратическое отклонение. m=',
                        ', n=', '.']
                    m = random.randint(2, 9)
                    n = 10 - m
                    s1 = z[0] + str(m) + z[1] + str(n) + z[2] + ' Ответ: ' + str(round(0 * (m / 10) + 1 * (n / 10) * (m / 9) + 2 * (n / 10) * ((n - 1) / 9) * (m / 8) + 3 * (n / 10) * ((n - 1) / 9) * ((n - 2) / 8) * (m / 7), 3)) + ' ' + str(round((0 * (m / 10) + 1 * (n / 10) * (m / 9) + 4 * (n / 10) * ((n - 1) / 9) * (m / 8) + 9 * (n / 10) * ((n - 1) / 9) * ((n - 2) / 8) * (m / 7)) - (0 * (m / 10) + 1 * (n / 10) * (m / 9) + 2 * (n / 10) * ((n - 1) / 9) * (m / 8) + 3 * (n / 10) * ((n - 1) / 9) * ((n - 2) / 8) * (m / 7)) ** 2,3)) + ' ' + str(round(((0 * (m / 10) + 1 * (n / 10) * (m / 9) + 4 * (n / 10) * ((n - 1) / 9) * (m / 8) + 9 * (n / 10) * ((n - 1) / 9) * ((n - 2) / 8) * (m / 7)) - (0 * (m / 10) + 1 * (n / 10) * (m / 9) + 2 * (n / 10) * ((n - 1) / 9) * (m / 8) + 3 * (n / 10) * ((n - 1) / 9) * ((n - 2) / 8) * (m / 7)) ** 2) ** (1 / 2), 3))
            if tem == 'nepr':
                s1 = 'Задачи отсутствуют'
            if tem == 'norm':
                s1 = 'Задачи отсутствуют'
        ...
        if sl == 'ur3':
            ...
            if tem == 'slozh-umn-ver':
                ...
                if run == 'z1':
                    ...
                    c = 1 - (b + q1 * q2 * q3 * q4)
                    s1 = z[0] + str(p1) + z[1] + str(p2) + z[2] + str(p3) + z[3] + str(p4) + z[4] + ' Ответ: ' + str(round(a, 3)) + ' ' + str(round(b, 3)) + ' ' + str(round(c, 3))
                if run == 'z2':
                    z = [
                ...
                if run == 'z3':
                    z = ['В каждой из трех урн содержится k1 = ', ' черных и k2 = ',
                         ' белых шара. Из первой урны наудачу извлечен один шар и переложен во вторую урну, после чего из второй урны наудачу извлечен один шар и переложен в третью урну. Найти вероятность того, что шар, наудачу извлеченный из третьей урны, окажется белым.']
            ...
            if tem == 'fpv':
                ...
                if run == 'z1':
                    z = [
                        ...
                        '% в третьей группе. Найти вероятность того, что случайно вызванный студент – отличник. Случайно вызванный студент оказался отличником. Найти вероятность того, что студент учится в третьей группе.']
                    ...
                    b = n / 100
                    c = k / 100
                    s1 = z[0] + str(m) + z[1] + str(n) + z[2] + str(k) + z[3] + ' Ответ: ' + str(round((3 / 8) * a + (1 / 4) * b + (3 / 8) * c, 3)) + ' ' + str(round((3 / 8) * c / ((3 / 8) * a + (1 / 4) * b + (3 / 8) * c), 3))
                if run == 'z2':
                    ...
                    pk = a / (a + b)
                    pl = b / (a + b)
                    s1 = z[0] + str(a) + z[1] + str(b) + z[2] + str(n) + z[3] + str(k) + z[4] + ' Ответ: ' + str(round(pk * c / (pk * c + pl * d), 3)) + ' ' + str(round(pl * d / (pk * c + pl * d), 3))
                if run == 'z3':
                    z = [
                    ...
                    p = 1 - d
                    g = 100 - k
                    s1 = z[0] + str(a) + z[1] + str(round(b,3)) + z[2] + str(n) + z[3] + str(o) + z[4] + str(k) + z[5] + str(g) + z[6] + ' Ответ: ' + str(round((a * c * p) / (a * c * p + b * l * d), 3))
            if tem == 'bayes':
                run = random.choice(['z1', 'z2', 'z3'])
                if run == 'z1':
                    ...
                    txt1 = 'Астрономический объект, за которым ведется наблюдение, может находиться в одном из двух состояний: Н1 или Н2. Априорные вероятности этих состояний P(Н1) = '
                    txt2 = ' Наблюдение ведется независимо двумя обсерваториями. Первая обсерватория обычно дает правильные сведения о состоянии наблюдаемого объекта в '
                    txt3 = ' Первая обсерватория сообщила, что объект находится в состоянии Н1, а вторая – что в состоянии Н2. Найти апостериорную вероятность состояния Н1. '
                ...
                if run == 'z2':
                    ...
                    txt3 = '. В конце дня, выбрав случайно один из подготовленных документов, руководитель констатировал, что в нём есть ошибка (событие A). Пользуясь формулой Байеса, выяснить, какова вероятность, что ошибку допустил первый чиновник, второй, третий.'
                    txt21 = ' вторая дает правильные сведения в '
                ...
                if run == 'z3':
                    ...
                    PB2 = (P2 * N2 / PA)
                    PB3 = (P3 * N3 / PA)
                    s1 += ' б) в общей массе бракованных деталей на конвейере доля первого станка составляет - ' + str(round(PB1, 3)) + ', второго – ' + str(round(PB2, 3)) + ', третьего – ' + str(round(PB3, 3))
            ...
            if tem == 'muavr-laplas':
                ...
                if run == 'z3':
                    z = [
                        'По результатам проверок налоговыми инспекциями установлено, что в среднем каждое второе малое предприятие региона имеет нарушение финансовой дисциплины. Найти вероятность того, что из ',
            ...
            if tem == 'int-muavr-laplas':
                ...
                if run == 'z1':
                    ...
                    while ((int(fb * 100) == 0) or (int(fa * 100) == 0) or (int(fc * 100) == 0)):
                        ...
                        c2 = (0 - n * p) / ((n * p * q) ** (0.5))
                        fc = (scipy.stats.norm.cdf(c1) - 0.5) - (scipy.stats.norm.cdf(c2) - 0.5)
                    s1 = z[0] + str(n) + z[1] + str(ma) + z[2] + str(mb) + z[3] + str(ma) + z[4] + str(mc) + z[5] + ' Ответ: ' + str(abs(round(fa, 3))) + ' ' + str(abs(round(fb, 3))) + ' ' + str(abs(round(fc, 3)))
                if run == 'z2':
                    ...
                    while ((int(f1 * 100) == 0) or (int(f2 * 100) == 0) or (int(f3 * 100) == 0)):
                        ...
                        f2 = abs(round(laplas(n, p, a2, b2), 3))
                        f3 = abs(round(laplas(n, p, a3, b3), 3))
                    s1 = z[0] + str(int(p * 100)) + z[1] + str(n) + z[2] + str(a1) + z[3] + str(b1) + z[4] + str(a2) + z[5] + str(b2) + z[6] + str(a3) + z[7] + str(b3) + z[8] + ' Ответ: ' + str(f1) + ' ' + str(f2) + ' ' + str(f3)
                if run == 'z3':
            ...
            if tem == 'puass':
                ...
                if run == 'z1':
                    ...
                    s1 = z[0] + str(m) + z[1] + str(n) + z[2] + str(k) + z[3] + str(k) + z[4] + str(k) + z[5] + ' Ответ: ' + str(round(pys, 3)) + ' ' + str(round(pys01, 3)) + ' ' + str(round(pys02, 3))
                if run == 'z2':
                    z = [
                ...
                if run == 'z3':
                    ...
                    while (m03 != 0):
                        ...
                        pys03 -= pys0
                    s1 = z[0] + str(n) + z[1] + str(t) + z[2] + str(m0) + z[3] + str(m1) + z[4] + str(m2) + z[5] + str(m3) + z[6] + ' Ответ: ' + str(round(pys01, 3)) + ' ' + str(round(pys02, 3)) + ' ' + str(round(pys03, 3))
            if tem == 'diskr':
                run = random.choice(['z1', 'z2'])
                if run == 'z1':
                    z = [
                        ...
                        ', для второго – ',
                        '. Пусть X - разность между числом удачных бросков первого и второго баскетболистов. Вычислить математическое ожидание, дисперсию и среднее квадратичное отклонение.']
            ...
            if tem == 'norm':
                ...
                if run == 'z1':
                    ...
                    s1 = z[0] + str(a) + z[1] + str(b) + z[2] + str(c) + z[3] + str(d) + z[4] + str(e) + z[5] + ' Ответ: ' + str(round(otvet, 3)) + ' ' + str(round(otvet1, 3))
                if run=='z2':
                ...
                if run == 'z3':
                    ...
                    while (int(otvet * 1000) == 0) or ((int(otvet1 * 1000) == 0)):
                        ...
                        f3 = scipy.stats.norm.cdf(s4)
                        otvet1 = round(abs(f2 - f3), 3)  # может быть отрицательным, поэтому берем модуль abs()
                    s1 = z[0] + str(a) + z[1] + str(b) + z[2] + str(c) + z[3] + str(d) + z[4] + str(e) + z[5] + ' Ответ: ' + str(otvet) + ' ' + str(otvet1)
            vop = int(request.GET.get('numbervop'))
            Task.objects.filter(nomer=vop).delete()
            Task(zadacha=s1, nomer=vop).save()
            return render(request, 'app/vibor.html',
